---
title: 設定を使い方に合わせる
description: Revclip 0.1.9の設定画面で履歴、外観、メニュー、ショートカットなどを変更する手順を説明します。
...
「設定」では、履歴の保存方法やメニューの見え方、起動時の動作を変更できます。Revclip 0.1.9（build 42）の設定画面には、一般、外観、メニュー、種類、除外アプリ、ショートカット、アップデート、エージェント設定、不具合報告、Panic の各ページがあります。

<!-- block: callout tone=warning -->
> **0.1.9の実機受入は未完了です。** 描画、貼り付け、連続操作の受入確認が残っています。コピー監視の間隔は500msで、次の観測より前に上書きされたコピーを記録できない場合があります。この間隔を変える設定はありません。
