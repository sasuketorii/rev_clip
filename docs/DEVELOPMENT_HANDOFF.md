...
- 通常サイドバーはセットアップ・一般・外観・ショートカット・faster OCR・プライバシー・アップデート・高度な設定・不具合報告・Panicです。Panic は末尾で文字とアイコンが黄色です。
- セットアップは「権限ステータス」「クリップボードメニュー」「FasterOCR」の順で、最初は権限を表示します。権限は既存 `RCPermissionsPreferencesController` を子controllerとして再利用し、切替時にviewを着脱します。権限の単独サイドバー項目は廃止し、旧 `showTab:@"permissions"` はセットアップの権限タブに転送します。未許可ステータスは赤色です。ショートカット部分は既存の RCHotKeyService の保存・競合チェックを共用します。メニューと OCR の保存済みショートカットを表示し、初期値は ⌘⇧V / ⌘⇧2 です。既存の設定を上書きしません。
...
- 競合は各設定画面の黄色ラベルに表示します。macOS 標準と Revclip 内の競合に加え、CleanShot X 5 の保存済み `LAVA*` ショートカット（JSON の `carbonKey` / `carbonModifiers`）を限定して照合します。他アプリ全般の登録は列挙できません。非排他登録が存在しても他プロセスの排他 probe は成功することを macOS 26 で実測したため、probe による誤判定・一時的な横取りは採用していません。
...
- 既存の`showTab:`識別子は維持します。高度な設定はmenu/type/agents、プライバシーはlinks/excludeです。別の設定保存層を作らず既存controllerを再利用します。
...
- CleanShot X 対応は設定形式に依存する互換アダプターです。設定キー数・件数・JSONサイズを制限し、ショートカット以外の値は読みません。無効な形式は無視します。起動中のCleanShot Xのみ照合し、アンインストール後の残存設定は拒否の根拠にしません。同じ値の再保存と機能のON/OFFは外部競合で阻止せず、保存値の検証は別に行います。将来のCleanShot形式変更や未保存の初期値まで検出できるという保証はありません。
- セットアップは保存値も読み取り専用で再検証します。入力拒否の警告は文脈が変わるまで維持し、自分自身のslotを競合扱いしません。テストの外部設定はfakeに注入し、利用者の実設定をfixtureにしません。
- メニュー表示中の⌘,は `RCHotKeyService` が追跡期間だけsession event tapで押下・解放を捕捉し、`performAfterMenuTrackingEnds:` で閉じてから設定を表示します。サブメニューだけの閉鎖では解除せず、最後のメニュー閉鎖・追跡終了通知・同期popup呼出しのreturn・owner破棄・全ホットキー解除で解放します。追跡モード外の入力は消費せずtapを撤去します。アクセシビリティ許可がない場合も通常の設定メニューをクリックできます。⌘Qなど他キーへの拡張はしていません。記録中・古いイベント・Panic中は実行しません。
- 履歴の種類アイコンは設定値8〜64ptに拡大・縮小し、styled行の高さも再計算します。画像サムネイルの拡大禁止と56×32pt描画枠は維持します。設定変更通知後のメニュー再構築経路は既存のdebounceを利用します。

- `CopySymbolicHotKeys` は⌘Cなどのアプリ標準操作を列挙しません。[Appleの標準ショートカット一覧](https://support.apple.com/ja-jp/102650)を根拠に、コピー・貼り付け・編集・一般的なウィンドウ操作を追加判定します。文字はレコーダー表示と共通のキーボード配列変換を利用します。保存済みの標準操作キーも警告し、起動時のグローバル登録は抑止します（保存値は上書きしません）。⌘⇧V / ⌘⇧2 は引き続き使えます。全アプリ固有のメニュー操作を網羅するものではありません。

- メニュー中の⌘,に一時Carbon登録を使うと、キー押下でメニューが先に閉じ、登録解除によってactionが届かないことを実機で確認しました。session tapでキー押下・解放を消費してから既存の追跡終了待ちへ渡します。テストではtapをfakeへ差し替えます。
