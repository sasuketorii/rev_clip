...
| `RCLinkPreviewService` | 未設定は手動。無効/手動/自動をユーザーが選択。手動の暗黙要求は通信しない。無効化は進行中/待機要求を中止しcacheを消去。不正な設定値は無効扱い | `RCLinkPreviewServiceTests` |
| `RCFastPreviewController` | 手動取得のOption-Pは表示中のハイライト行だけ。hideでmonitorを撤去。非同期完了は世代と弱参照を検査 | `RCFastPreviewTests` |
| `RCPreferencesWindowController` | サイドバー7項目。高度な設定/プライバシーの子ページは上タブ。従来のページIDを維持し直接遷移を壊さない | `RCPreferencesSidebarTests` |
...
リンク取得は外部サイトにIPアドレスやURLを伝えるため、開発版の既定を手動にしています。一度の明示取得は他のURLを自動取得する許可になりません。取得済みのメモリcacheは手動モードでも再利用できます。自動取得へ切り替えるとメニュー/ホバーから取得します。
