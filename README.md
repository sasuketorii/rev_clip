...
v0.1.8の終了停止修正・既存機能品質修正と、その測定結果・残る確認は[品質検証報告](docs/QUALITY_REPORT.md)を参照してください。

次版のユーザーテスト候補 **0.1.9 (42)** では、コピー直後のショートカット表示を取得・保存の完了と同期します。メニューバーから開いた場合は、まだ項目を選択しておらずサブメニューも開いていないときに限り、保存後に一覧を更新します。選択中の行は動かしません。表示待ち中の別入力、前面アプリ変更、監視の停止・再開では古い表示要求を破棄します。500msの監視既定値は維持し、観測前に上書きされたコピーを回収する保証はありません。Release合成回帰308件が成功していますが、実際の描画・貼り付け・連続操作の受入は未完了です。公開版とローカル候補は区別してください。
