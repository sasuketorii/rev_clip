...
**⌘⇧Vで、履歴も、定型文も、設定も。** Revclipは、macOSのメニューバーに常駐するクリップボードマネージャーです。コピー履歴を辿り、スニペットの内容をプレビューして、そのまま貼り付ける。いつもの作業を、ひとつのメニューから続けられます。
...
| **履歴はローカルに** | アカウント登録も、クラウドのセットアップも不要。履歴・スニペットをMacに保存します。 |
| **10件ずつ、迷わず辿る** | 履歴は既定で10件ごとのフォルダに整理。件数やメニュー表示は設定できます。 |
| **繰り返す文章をスニペットに** | フォルダで分類し、専用エディタで編集。インポート・エクスポートにも対応します。 |
...
2. **⌘⇧V**でメインメニューを開き、履歴またはスニペットを選びます。
3. 元のアプリへ貼り付けます。自動ペーストを無効にすれば、クリップボードへのコピーだけにもできます。

| 入口 | 開くもの |
| --- | --- |
| **⌘⇧V** | 履歴・スニペット・履歴消去・設定・テンプレート編集・終了 |
| **⌘⌃V** | 履歴とアプリ操作。スニペットを含まないメニュー |
| **メニューバーのアイコン** | メインメニュー |
...
| スニペットエディタ | [SnippetEditor](src/Revclip/Revclip/UI/SnippetEditor) |
