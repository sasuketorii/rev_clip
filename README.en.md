...
For a scripted background launch without the menu, use `open -g path/to/Revclip.app --args -suppressLaunchMenu YES`. On first launch, `open -g` alone cannot be distinguished from an ordinary open. The v0.2.3 release target `33138dd` passed Woodpecker #13 and macOS CI, including 429 XCTest cases in each of Debug and Release; 25 focused update tests also passed locally. An isolated real Sparkle app verified scheduled reminders, opening the update window in front, and reminder cleanup. Public delivery and real-device coverage are recorded separately in the [quality report](docs/QUALITY_REPORT.md).
