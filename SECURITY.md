...
### Temporary keyboard handling

While a shortcut is being recorded, Revclip temporarily intercepts key codes and modifiers to prevent the recorded command from running. While a Revclip menu is open, a separate temporary handler recognizes Command-comma and opens Settings after the menu closes. These handlers require Accessibility permission, do not store or transmit keystrokes, and are removed when recording or menu tracking ends. Other keys pass through the menu handler. Without permission, Settings remains available by clicking its menu item.
