...
- 履歴・テンプレートをクラウド同期する機能はありません。アップデート確認・ダウンロードと、URL単体のリンクプレビュー・ファビコン取得はネットワークを利用します。既定は自動取得です。メニュー表示・ホバー時に取得します。設定の「プライバシー → リンク」で手動取得または取得しないを選べます。手動取得では⌥を押しながらリンクにカーソルを合わせた場合だけ取得します。取得時にはIPアドレスやURL全体がサイトに伝わり得ます。許可の取り消しは進行中の取得を中止し、キャッシュを消去します。文章に混在するURLではリンク取得を行いません。
