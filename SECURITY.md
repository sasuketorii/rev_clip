...
- 履歴・テンプレートをクラウド同期する機能はありません。アップデート確認・ダウンロードと、URL単体のリンクプレビュー・ファビコン取得はネットワークを利用します。既定は手動取得です。メニューを開く・ホバーするだけでは通信せず、リンクを選んで⌥Pを押した場合だけ取得します。設定の「プライバシー → リンク」で自動取得または取得しないを選べます。取得時にはIPアドレスやURL全体がサイトに伝わり得ます。許可の取り消しは進行中の取得を中止し、キャッシュを消去します。文章に混在するURLではリンク取得を行いません。
...
Revclip has no cloud history synchronization. Update checks and downloads, plus link previews and favicon retrieval for standalone web URLs, use the network. Manual fetching is the default: opening a menu or hovering does not connect. Highlight a link and press Option-P to fetch it, or choose automatic/disabled fetching under Privacy → Links. Fetching can expose your IP and full URL to the website. Changing permission cancels pending requests and clears cached previews. URLs embedded in prose do not trigger link retrieval. Developer ID signing and Apple notarization are distribution checks, not proof that an application has no vulnerabilities. Revclip does not claim third-party security certification or a comprehensive independent audit.
